...
class Curve:
# TO DO implement OIS dual-curve bootstrapping
# TO DO finish doc-string documentation
    def __init__(self, curve, curve_date, conventions, instruments, market_data):
        self.name = curve
        self.curve_date = curve_date

        self.settlement_date = curve_date + \
            qlib.Period(
                int(conventions[self.name]['deposits_SpotLag']), qlib.Days)
        ...
        self.deposits = Deposits(self, conventions)
        self.futures = Futures(self, conventions)
        self.swaps = Swaps(self, conventions)
    ...
    def build_libor_curve(self):
        ...
        for date in self.outputs.qlibcurve.dates():
            self.outputs.dates.append(date.ISO())
        for date in  self.outputs.qlibcurve.dates():
    ...
    def get_deposits_rate_helpers(self, curve):
        ...
        Returns:
            deposit_rate_helpers (list): list of QuantLib DepositRateHelper
                                         objects.
        """
        print(type(self.settlement_days))
...
def import_data():
    ...
    conventions = pandas.read_csv('conventions.csv', index_col=0)
    instruments = pandas.read_csv('instruments.csv', index_col=0)
    market_data = pandas.read_csv('market_data.csv', index_col=0)
    all_curves = pandas.read_csv('curves_to_build.csv', index_col=0)
...
def main():
    ...
    for curve in curves_to_build:
        ...
        curve = Curve(curve,
                      ...
                      instruments,
                      market_data)
        curve.build_libor_curve()
        print(curve.outputs.pandas_curve)
        print(curve.outputs.iso_dates)
